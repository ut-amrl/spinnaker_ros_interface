 ...
 *maximums to width and height. It also shows the setting of a new pixel format,
 *which is an enumeration type node.
 *
 *	Following this, we suggest familiarizing yourself with the Exposure
 ...
 *comfortable with Exposure and ImageFormatControl, we suggest checking out any
 *of the longer, more complicated examples related to camera configuration:
 *ChunkData, LookupTable, Sequencer, or Trigger.
 */
...
// X and Y, width, height, and pixel format. These settings must be applied
// before BeginAcquisition() is called; otherwise, they will be read only. Also,
// it is important to note that settings are applied immediately. This means if
// you plan to reduce the width and move the x offset accordingly, you need to
// apply such changes in the appropriate order.
int ConfigureCustomImageSettings(INodeMap& nodeMap) {
  int result = 0;

  cout << endl
       << endl
       << "*** CONFIGURING CUSTOM IMAGE SETTINGS ***" << endl
       << endl;
...
int AcquireImages(CameraPtr pCam,
  ...
  try {
    ...
    if (IsAvailable(ptrStringSerial) && IsReadable(ptrStringSerial)) {
      ...
      cout << "Device serial number retrieved as " << deviceSerialNumber
           << "..." << endl;
    }
    cout << endl;
